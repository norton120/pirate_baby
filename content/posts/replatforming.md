...
Reflection amongst myself and the client team helped us come to what I now am confident was the right decision. Our "production replatform" became an iterative release of production features coupled with refactorings. Initially, that means only a single operation will be executed in our refactored framework - an operation that is trivial to implement in the new framework, but very complex and difficult in the legacy platform. Once successful, we will address individual operations that lack in performance, test coverage and observability 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwNzE5OTE0OTIsMTk5NDU3MDYwNSw4NT
