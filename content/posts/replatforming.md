...
Reflection amongst myself and the client team helped us come to what I now am confident was the right decision. Our "production replatform" became an iterative release of production features coupled with refactorings. Initially, that means only a single operation will be executed in our new framework - an operation that is trivial to implement in the new framework, but very complex and difficult in the legacy platform. Once successful, we will address individual operations lacking in performance, test coverage and observability with small, precision refactorings. These may result in our porting some other operations to the new framework, where the resulting 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTczNzM3MjM0NCwxOTk0NTcwNjA1LDg1OD
